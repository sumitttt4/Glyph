...
// The Monogram Engine: Creates unique logo marks by combining Shape + Letter
// Multiple styles based on vibe: contained, knockout, stacked, icon-only
function MonogramMark({ brand, color, bg }: { brand: BrandIdentity, color: string, bg: string }) {
    const letter = brand.name.charAt(0).toUpperCase();
    const vibe = brand.vibe.toLowerCase();

    // Determine logo style based on vibe
    const getLogoStyle = () => {
    ...
    const uniqueId = `logo-${brand.id?.slice(0, 8) || 'default'}`;

    // Style 1: CONTAINED - Letter inside shape with fill
    if (style === 'contained') {
        return (
            <div className="relative w-full h-full flex items-center justify-center">
                {/* Shape as filled background */}
                <svg
                ...
                {/* Letter centered */}
                <span
                    className="relative z-10 text-6xl font-bold leading-none select-none"
                    style={{ color: bg, fontFamily: 'var(--font-manrope)', textShadow: '0 2px 8px rgba(0,0,0,0.1)' }}
                >
                    {letter}
                </span>
            </div>
    ...
    // Style 2: KNOCKOUT - Letter punched through shape (SVG mask)
    if (style === 'knockout') {
        return (
            <div className="relative w-full h-full flex items-center justify-center">
                <svg viewBox="0 0 100 100" className="w-[90%] h-[90%]">
                    ...
                    {/* Transform shape path to fit 0-100 viewbox */}
                    <g transform="translate(12, 12) scale(3.2)" mask={`url(#${uniqueId}-mask)`}>
    ...
    // Style 3: STACKED - Icon above letter wordmark
    if (style === 'stacked') {
        return (
            <div className="relative w-full h-full flex flex-col items-center justify-center gap-2">
                ...
                <span
                    ...
                    style={{ color: color, fontFamily: 'var(--font-manrope)' }}
                >
    ...
    // Style 4: ICON-BADGE - Shape with small letter badge
    if (style === 'icon-badge') {
        return (
            <div className="relative w-full h-full flex items-center justify-center">
                {/* Main shape */}
                ...
                {/* Corner badge with letter */}
                <div
                    className="absolute bottom-[15%] right-[15%] w-10 h-10 rounded-full flex items-center justify-center shadow-lg"
