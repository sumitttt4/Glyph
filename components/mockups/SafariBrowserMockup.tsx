 ...
 */
export const SafariBrowserMockup = ({ brand, className }: SafariBrowserMockupProps) => {
    const tokens = brand.theme.tokens.light;

    return (
        <div className={`rounded-xl overflow-hidden border border-stone-300 shadow-lg bg-stone-100 ${className}`}>
            {/* macOS Window Titlebar */}
            <div className="h-11 bg-gradient-to-b from-[#E8E8E8] to-[#D5D5D5] flex items-center px-4 border-b border-stone-300 gap-2 relative">
                {/* Traffic Lights */}
                ...
                <div className="flex-1 flex items-center justify-center mx-8">
                    <div className="bg-white rounded-lg shadow-sm border border-stone-200 px-4 py-2 flex items-center gap-3 w-full max-w-md">
                        {/* Favicon */}
                        <div
                            className="w-5 h-5 rounded flex items-center justify-center shrink-0"
                            style={{ backgroundColor: tokens.primary }}
                        >
                            <svg viewBox={brand.shape.viewBox || "0 0 24 24"} className="w-3 h-3">
                                <path d={brand.shape.path} fill="#FFFFFF" />
                            </svg>
                        </div>
                        {/* URL */}
